<template>
  <div class="container">
    <div class="inner">
      <a class="return-link" href="javascript:void(0)" rel="noreferrer noopener" @click="handleError">返回首页</a>

      <svg class="bear-logo bear-logo--tears" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
        <g transform="matrix(1.34105 0 0 1.34105 -51.157 -1049.694)">
          ...
          ></path>
          <path
            d="M179.114 931.763c0 7.657-19.04 24.493-30.27 24.493s-32.117-16.836-32.117-24.493 20.888-12.477 32.118-12.477 30.27 4.82 30.27 12.477z"
          ></path>
          <ellipse ry="23.111" rx="23.762" cy="827.682" cx="68.304" fill="#803300"></ellipse>
          <path
            ...
            fill="#e9c6af"
          ></path>
          <ellipse transform="scale(-1 1)" cx="-231.243" cy="827.682" rx="23.762" ry="23.111" fill="#803300"></ellipse>
          <path
      ...
      <h1 class="glitchy-text error__code">
        <span class="glitchy-text__char--readable">{{ CODE }}</span
        ><span
          v-for="(char, idx) of CODE"
          :key="`char_${idx}`"
          class="glitchy-text__char"
          aria-hidden="true"
          :data-char="char"
          :style="getStyle()"
          >{{ char }}</span
      ...
      <h1 class="glitchy-text error__code-message">
        <span class="glitchy-text__char--readable">{{ CODE_MSG }}</span
        ><span
          v-for="(char, idx) of CODE_MSG"
          :key="`msg_${char}_${idx}`"
          class="glitchy-text__char"
          aria-hidden="true"
          :data-char="char"
...
const update = (e) => {
  const el = root.value.querySelector('.container');
  if (e.acceleration && e.acceleration.x !== null) {
    el?.style.setProperty('--X', e.acceleration.x);
    el?.style.setProperty('--Y', e.acceleration.y);
  } else {
    el?.style.setProperty('--X', e.pageX / window.innerWidth - 0.4);
    el?.style.setProperty('--Y', e.pageY / window.innerHeight - 0.4);
  }
};
useEventListener(root, 'mousemove', update);
useEventListener(window, 'devicemotion', update);
...
.container {
  ...
  .bear-logo,
  .error__code span:nth-of-type(2):after,
  .error__code span:nth-of-type(3):after,
  .error__code span:nth-of-type(4):after,
  .error__code-message span:nth-of-type(2):after,
  .error__code-message span:nth-of-type(3):after,
  .error__code-message span:nth-of-type(4):after,
  .error__code-message span:nth-of-type(6):after,
  .error__code-message span:nth-of-type(7):after,
  .error__code-message span:nth-of-type(8):after,
  .error__code-message span:nth-of-type(9):after,
  .error__code-message span:nth-of-type(10):after {
